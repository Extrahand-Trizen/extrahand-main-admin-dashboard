...
import { listPaymentRefunds } from "@/lib/api/payments";
...
export default function PaymentRefundsPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => listPaymentRefunds({ limit: 100, offset: 0 }),
        ...
        <CardContent>
          <div className="rounded-md border border-gray-200 overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-500">No refunds found</td></tr>
                ) : rows.map((row) => (
