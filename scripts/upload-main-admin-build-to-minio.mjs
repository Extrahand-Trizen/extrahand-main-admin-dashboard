/**
 * Upload Main Admin Dashboard STATIC export (`out/`) to MinIO.
 *
 * Target layout:
 ...
 *
 * Usage:
 *   node scripts/upload-main-admin-build-to-minio.mjs
 *   node scripts/upload-main-admin-build-to-minio.mjs --version 2
 *   node scripts/upload-main-admin-build-to-minio.mjs --version 2 --dry-run
 *
 * Credentials: scripts/.env.minio-builds (or process env / GitHub Secrets)
 */
...
const APP_FOLDER = String(
  process.env.APP_FOLDER || 'main-admin-dashboard',
).trim();
...
async function main() {
  ...
  console.log('══════════════════════════════════════════════════');
  console.log(' Main Admin Dashboard → MinIO (static export)');
  console.log('══════════════════════════════════════════════════');
  ...
  for (const filePath of files) {
    ...
    // Custom domains request keys without index.html.
    // Next.js trailingSlash + Link prefetch use BOTH `/path` and `/path/`.
    // Publish HTML at both keys so soft-nav prefetch does not 404 in the console.
    if (relative === 'index.html' || relative.endsWith('/index.html')) {
      const bareRelative =
        relative === 'index.html' ? '' : relative.slice(0, -'/index.html'.length);
      if (!bareRelative) continue; // root is usually served via index document rules

      const aliases = [
        `${remoteBuildPrefix}/${bareRelative}`,
        `${remoteBuildPrefix}/${bareRelative}/`,
      ];

      for (const aliasObjectName of aliases) {
        try {
          if (dryRun) {
            console.log(`   (dry-run) ${aliasObjectName}  ← spa route alias`);
          } else {
            await client.fPutObject(BUCKET_NAME, aliasObjectName, filePath, {
              'Content-Type': 'text/html; charset=utf-8',
              'Cache-Control': cacheControlFor(filePath),
            });
            console.log(`   ✅ ${aliasObjectName}  ← spa route alias`);
          }
          uploaded += 1;
        } catch (err) {
          failed += 1;
          console.error(`   ❌ ${aliasObjectName}: ${err?.message || err}`);
        }
      }
    }
  }
  ...
  const browserUrl = String(process.env.MINIO_BROWSER_REDIRECT_URL || '').replace(/\/+$/, '');
  console.log('\n══════════════════════════════════════════════════');
  ...
  if (browserUrl) {
    console.log(`Browser redirect:    ${browserUrl}`);
  }
  console.log('══════════════════════════════════════════════════');
